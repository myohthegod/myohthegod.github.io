import pgzrun # Python Pygame Zero run module (allows you to run the program)
...
def draw():  # Pygame Zero Draw function that draws everything on the screen
    ...
    screen.draw.text("Coins: " + str(Coins),
                     fontsize=55,
                     color="black",
                     topleft=(6, 6))
    screen.draw.text("$" + str(cost1),
                     fontsize=55,
                     color="black",
                     topright=(670, 90))
    screen.draw.text("$" + str(cost2),
                     fontsize=55,
                     color="black",
                     topright=(670, 290))
    screen.draw.text("$" + str(cost3),
                     fontsize=55,
                     color="black",
                     topright=(670, 490))
    screen.draw.text("$" + str(cost4),
                     fontsize=55,
                     color="black",
                     topright=(170, 90))
    screen.draw.text("$" + str(cost5),
                     fontsize=55,
                     color="black",
                     topright=(195, 290))
    screen.draw.text("$" + str(cost6),
                     fontsize=55,
                     color="black",
                     topright=(230, 490))
    if Coins >= cost1:  # Tells you if you have enough money for one of the buttons that increase your coins
        screen.draw.text("Purchasable!",
                         fontsize=45,
                         color="blue",
                         topright=(676, 60))
    if Coins >= cost2:
...
def reset_coin():  # Resets the coin to its default size after clicking
    global coin
    coin.image = "coin"
    return
...
def on_mouse_down(pos):  # Pygame Zero handle mouse click function
    ...
    if button.collidepoint(pos):  # Checks if player clicks on buttons
        if Coins >= cost1:
            ...
            cost1 += random.randint(
                12, 20
            )  # Increase the cost of that button by a random amount between 12 and 20
            clock.schedule_interval(
                add_coins, 3.75)  # Adds coins to the amount of coins you have
    ...
    if button2.collidepoint(pos):
        if Coins >= cost3:
            ...
            Coins = Coins - cost3
            cost3 += random.randint(120, 175)
            clock.schedule_interval(add_more_coins, 1.5)
    if button3.collidepoint(pos):
        if Coins >= cost4:
            button3.image = "button-down"
            Coins = Coins - cost4
            cost4 += random.randint(222, 350)
            clock.schedule_interval(add_more_coins, 0.75)
    if button4.collidepoint(pos):
        if Coins >= cost5:
            button4.image = "button-down"
            Coins = Coins - cost5
            cost5 += random.randint(450, 500)
            clock.schedule_interval(add_lots_coins, 0.5)
    if button5.collidepoint(pos):
        if Coins >= cost6:
            button5.image = "button-down"
            Coins = Coins - cost6
            cost6 += random.randint(1200000, 3000000)
            clock.schedule_interval(add_huge_coins, 0.1)
    return
